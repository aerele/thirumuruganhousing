...
def absconded_entry(doc):
	...
	rent_bal = frappe.db.sql('''select tenant_name,sum(outstanding) from `tabTenant Payment` where outstanding > 0 and tenant_name=%s ''', (doc.tenant_name), as_dict=1)
	frappe.msgprint("Linked To Absconed Tenant")
	abs_doc=frappe.new_doc('Absconding Tenant')
